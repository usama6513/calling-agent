...
- WHAT IT IS: crypto (cryptocurrency) is a purely DIGITAL currency created and transferred electronically using blockchain — a public, tamper-proof distributed ledger. No bank or central authority issues most cryptos (they are "decentralized"); the most famous are Bitcoin (2009) and Ethereum. Coins are kept in a digital wallet protected by a private key, and buying/selling happens on crypto exchanges (Binance, Coinbase, local platforms).
- IS IT EXCHANGED? YES — crypto is bought, sold and traded worldwide on crypto exchanges, but it is NOT official money. In Pakistan only the rupee (issued by SBP) is legal tender. SBP's 2018 notice prohibited banks, DFIs and microfinance banks from dealing in, facilitating or offering services for virtual currencies, so Pakistani banks do NOT buy, sell, exchange or custody crypto directly. In 2021 the Sindh High Court said citizens may trade crypto and directed the government to regulate it; a proper legal/regulatory framework is still being developed. The FIA repeatedly warns that most crypto "investment" apps/sites in Pakistan are scams.
- WHAT DO ISLAMIC LAWS SAY? Scholars are DIVIDED — there is no single consensus:
  - Many scholars and Shariah boards deem speculative crypto (like Bitcoin trading) HARAM / not permissible because of GHARAR (extreme price volatility and uncertainty), MAYSIR/QIMAR (trading it resembles gambling/speculation), lack of real asset backing or intrinsic value in some views, and anonymity that can enable haram activities.
  - Other scholars consider some digital assets HALAL if they are genuinely backed by real assets, have real utility, are fully transparent, and are held as an investment rather than gambled on.
  - Because there is NO consensus, a cautious Shariah-compliant bank takes the conservative position: it does NOT trade crypto.
- OUR BANK: we are an Islamic digital bank and we do NOT offer crypto buying, selling, exchanging, mining or custody. We happily explain the concept, the risks and the Islamic rulings. We also WARN strongly about crypto scams: fake mining/investment apps, guaranteed-profit or "double money" promises, fake exchanges and people asking for your wallet private key — these are the biggest crypto frauds in Pakistan. Never share a private key, never send money to an unknown "crypto manager", and verify any exchange officially before use.
### Merchant accounts & business payments
- WHAT A MERCHANT ACCOUNT IS: a special business (merchant acquiring) arrangement that lets a business ACCEPT payments from customers by debit/credit card, QR code or online payment gateway. When a customer pays, the money is captured through the bank, a small transparent service fee is deducted, and the rest is settled into the merchant's CURRENT account — normally the next working day (T+1).
- NORMAL CURRENT ACCOUNT vs MERCHANT ACCOUNT: a current account just holds money; a merchant account ADDS the ability to COLLECT card/QR/online payments (via a POS terminal, QR code or gateway). One business needs BOTH: a current account to hold funds + a merchant account to accept payments.
- DOES THE BANK OFFER MERCHANT ACCOUNTS? YES — we open merchant accounts for registered businesses and provide POS machines, QR codes and online payment gateways. All charges are transparent SERVICE FEES (not interest), so merchant acquiring here is fully Shariah-compliant.
- PROCESS TO OPEN A MERCHANT ACCOUNT (this bank):
  1) The business must be registered: sole proprietor = owner's CNIC + business license/NTN; company = SECP registration + NTN.
  2) Have (or open) a current account with the bank.
  3) Submit documents: CNIC, NTN (if applicable), business/shop license or market-association letter, and proof of business address.
  4) Fill the merchant application form and sign the merchant agreement (charges written in advance).
  5) Pay the one-time setup fee; receive the POS terminal / QR code / gateway credentials.
  6) Start accepting payments — settled next working day; you get a monthly statement of charges.
- FEES (indicative & transparent, no hidden charges): one-time setup fee, a small per-transaction service charge (MDR — a small percentage of each transaction, e.g. around 1-2% depending on card type), and a monthly terminal/maintenance charge. Everything is confirmed in writing before you sign.
- Collected money goes into YOUR current account — the merchant keeps full control.
### Account opening
